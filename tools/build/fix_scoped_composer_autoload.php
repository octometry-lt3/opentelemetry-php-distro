...
// --- Fix 4: Expose PHP 8.2 global function polyfills needed by scoped packages ---
// PHP 8.1 has no native \ini_parse_quantity(), but scoped upstream packages can still call
// it as a PHP-native global (for example opentelemetry-metrics-runtime's MemoryMetrics). The
// scoped Symfony polyfill defines OTelDistroScoped\ini_parse_quantity(); this tiny autoloaded
// alias preserves the PHP 8.2 global function contract for PHP 8.1 without reimplementing it.

$polyfillAliasFileName = 'otel_distro_php82_polyfill_aliases.php';
$polyfillAliasFilePath = $composerDir . $polyfillAliasFileName;
$polyfillAliasFileContent = <<<PHP
<?php

declare(strict_types=1);

// Generated by fix_scoped_composer_autoload.php.
// Expose PHP 8.2 native-compatible global polyfills for scoped packages on PHP 8.1.
if (!\\function_exists('ini_parse_quantity') && \\function_exists('\\{$prefix}\\ini_parse_quantity')) {
    function ini_parse_quantity(string \$shorthand): int
    {
        return \\{$prefix}\\ini_parse_quantity(\$shorthand);
    }
}

PHP;

if (file_put_contents($polyfillAliasFilePath, $polyfillAliasFileContent) === false) {
    fwrite(STDERR, "Failed to write PHP 8.2 polyfill aliases: $polyfillAliasFilePath\n");
    exit(1);
}
fwrite(STDOUT, "Wrote PHP 8.2 polyfill aliases: $polyfillAliasFilePath\n");

$insertPolyfillAliasAfter = static function (string $filePath, string $needle, string $entry) use ($polyfillAliasFileName): void {
    if (!is_file($filePath)) {
        fwrite(STDERR, "File does not exist (skipping PHP 8.2 polyfill alias autoload): $filePath\n");
        return;
    }
    ...
    if (str_contains($content, $polyfillAliasFileName)) {
        fwrite(STDOUT, "PHP 8.2 polyfill alias autoload already present in: $filePath\n");
        return;
    }

    $needlePos = strpos($content, $needle);
    if ($needlePos === false) {
        fwrite(STDOUT, "Skipping PHP 8.2 polyfill alias autoload (polyfill marker not found) in: $filePath\n");
        return;
    }

    $insertPos = strpos($content, ',', $needlePos);
    if ($insertPos === false) {
        fwrite(STDERR, "Failed to find insertion point after PHP 8.2 polyfill entry in: $filePath\n");
        exit(1);
    }
    ++$insertPos;

    if (file_put_contents($filePath, substr($content, 0, $insertPos) . $entry . substr($content, $insertPos)) === false) {
        fwrite(STDERR, "Failed to add PHP 8.2 polyfill alias autoload to: $filePath\n");
        exit(1);
    }

    fwrite(STDOUT, "Added PHP 8.2 polyfill alias autoload to: $filePath\n");
};

$polyfillAliasHash = md5($prefix . ':otel-distro:php82-polyfill-aliases');
$insertPolyfillAliasAfter(
    $composerDir . 'autoload_files.php',
    '/symfony/polyfill-php82/bootstrap.php',
    " '$polyfillAliasHash' => \$vendorDir . '/composer/$polyfillAliasFileName',"
);
$insertPolyfillAliasAfter(
    $composerDir . 'autoload_static.php',
    '/symfony/polyfill-php82/bootstrap.php',
    " '$polyfillAliasHash' => __DIR__ . '/$polyfillAliasFileName',"
);
